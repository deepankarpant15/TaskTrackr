...
public List<Todo> getTodosByUsername(String username) {
    List<Todo> todos = new ArrayList<>();
    String sql = "SELECT id, title, description, target_date FROM todos WHERE username = ?";
    ...
    try (Connection conn = JDBCUtils.getConnection();
        ...
        while (rs.next()) {
            ...
            Timestamp timestamp = rs.getTimestamp("target_date");
            if (timestamp != null) {
                todo.setTargetDate(timestamp.toLocalDateTime().toLocalDate()); // Conversion
            } else {
                todo.setTargetDate(null); // Handle null values gracefully
            }
...
public List<Todo> getAllTodos() {
    List<Todo> todos = new ArrayList<>();
    // Assuming you have a JDBC utility class to manage DB connections
    try (Connection conn = JDBCUtils.getConnection()) {
